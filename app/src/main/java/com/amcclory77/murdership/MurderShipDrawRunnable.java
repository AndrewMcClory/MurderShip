...
public class MurderShipDrawRunnable implements Runnable {
	...
	public MurderShipDrawRunnable(MurderShipGameData gameData, SurfaceHolder surfaceHolder) {
		...
		mAlertLevelPaint.setTextSize(mBitmaps.mTileHeight/2);
    ...
    public void run() {
    	...
    	while(mIsRunning)
    	{
    		...
    		if (mActiveDrawBuffer.mIsValid) {
				...
				final int i = Log.i("INFO:", "FPS: " + (mNumFrames * 1000) / elapsedTime + " Frames: " + mNumFrames + " Times in ms: " + elapsedTime);
				mNumFrames = 0;
				mStartTime = System.currentTimeMillis();
			}
    	}
    	...
    	while (tilesToCheckX > 0)
    	{
    		...
    		{	
    			if ((curDeltaX != 0 || curDeltaY != 0) && curTileX < mGameData.mMapWidthTiles && curTileY < mGameData.mMapHeightTiles)
    			{
					...
					srcRect = new Rect();
					srcRect.top = 0;
					srcRect.bottom = mBitmaps.mRoomBitmap[mGameData.mTiles[curTileX][curTileY]].getHeight() - 1;
					srcRect.left = 0;
					srcRect.right = mBitmaps.mRoomBitmap[mGameData.mTiles[curTileX][curTileY]].getWidth() - 1;

					Rect dstRect = new Rect();
					dstRect.top = curPixelY;
					dstRect.bottom = curPixelY + mBitmaps.mRoomBitmap[mGameData.mTiles[curTileX][curTileY]].getHeight();
					dstRect.left = curPixelX;
					dstRect.right = curPixelX + mBitmaps.mRoomBitmap[mGameData.mTiles[curTileX][curTileY]].getWidth();*/

    				canvas.drawBitmap(mBitmaps.mRoomBitmap[mGameData.mTiles[curTileX][curTileY]], curPixelX, curPixelY, null);
					//canvas.drawBitmap(mBitmaps.redblockbitmap, curPixelX, curPixelY, null);
    			}
