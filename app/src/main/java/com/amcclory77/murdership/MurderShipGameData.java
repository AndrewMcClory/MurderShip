...
public class MurderShipGameData {
	...
	static public class GameMap implements Serializable {
			...
			static public class AnimateObject extends LocationObject implements Serializable {
				...
				public AnimateObject(int left, int bottom, int type, int speed, int bitmap, int conversationIndex, long minTimeForUpdate) {
					mLeft = left;
					mBottom = bottom;
					mType = type;
					mSpeed = speed;
			...
			void generateRandomGameObjects() {
				...
				mTargetIndex = 0;
				mGameObjects[mTargetIndex] = new AnimateObject(0, 0,
				...
				mGameObjects[mHeroIndex] = new AnimateObject(0, 0,
						AnimateObject.HERO_TYPE, mTileWidth/4, 0, -1, 45);
				++mNumGameObjects;
				...
				for (int i = 0; i < 10; ++i)
				{
					mGameObjects[mNumGameObjects++] = new AnimateObject(0, 0,
							AnimateObject.SUSPECT_TYPE, mTileWidth/8, i + 1, i, 45);
				}
			...
			for (int y = roomDesc.mTopRoomSection; y < roomDesc.mTopRoomSection + roomDesc.mHeightRoomSections; ++y) {
				for (int i = 0; i < mMap.mRoomWidthInTiles; ++i) {
					for (int j = 0; j < mMap.mRoomHeightInTiles; ++j) {
						...
						mObstacles[x * mMap.mRoomWidthInTiles + i][y * mMap.mRoomHeightInTiles + j] = mTiles[x * mMap.mRoomWidthInTiles + i][y * mMap.mRoomHeightInTiles + j] == roomDesc.mWallBitmap;
					}
				}
			}
		}
		...
		while (pathNodesAdded > 0) {
			...
			for (xTile = leftTile; xTile < rightTile; ++xTile) {
				for (yTile = topTile; yTile < bottomTile; ++yTile) {
					...
					if (mObstacles[xTile][yTile]) {
						continue;
					} else {
						if ((curPathNode.mDistanceFromStart != -1)) {
                            continue;
                        }
					}
